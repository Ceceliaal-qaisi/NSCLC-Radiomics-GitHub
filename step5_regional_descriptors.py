...
# Convex hull is calculated from the CORNERS of the
# foreground pixels rather than pixel centers.
#
# This makes the hull area geometrically comparable
# with the binary-region area and guarantees:
#
#       Convex Hull Area >= Region Area
#
# Therefore:
#
#       Solidity = Area / Convex Hull Area
#
# is physically meaningful and should be in [0, 1].
# ============================================================
...
def get_boundary(mask2d):
    ...
    A foreground pixel belongs to the boundary if at least
    one of its four-neighbours is background.
    """
...
def perimeter_from_mask(mask2d):
    """
    Calculate perimeter from exposed horizontal and
...
def hu_moments(mask2d):
    ...
    Hu moments are constructed from normalized central
    moments and are theoretically invariant to:
...
def eccentricity(mask2d):
    """
    Eccentricity calculated from the eigenvalues of
...
def pixel_corners(mask2d):
    """
    Convert every foreground pixel into its four corners.

    A pixel centered at (y, x) is represented by:
    ...
    The convex hull of these corners represents the
    geometric extent of the binary region.
    """
...
def convex_hull_area(mask2d):
    """
    Calculate convex hull area from the actual geometric
    extent of the foreground pixels.

    IMPORTANT:

    The hull is constructed from pixel CORNERS instead
    of pixel centers.

    This avoids the previous problem where:

        Hull Area < Region Area

    could occur.

    The resulting value is geometrically compatible
    with the binary region area.
    """
...
def calculate_regional_descriptors(mask2d):
    ...
    #
    # Circle -> 1
    # More irregular shapes -> larger value
    # --------------------------------------------------------
    ...
    if hull_area > 0:
        ...
        # after using pixel-corner hulls.
...
for index in tumor_slices:
    ...
    descriptors = (
        calculate_regional_descriptors(
            clean_mask
        )
    )
...
with open(
    ...
    f.write(
        "The convex hull is calculated from the "
        "corners of foreground pixels using the "
        "monotonic chain algorithm and the "
        "shoelace formula.\n\n"
    )
...
def rotate_mask(
    ...
    """
    Rotate binary mask around its image center.
    """
...
def scale_mask(
    ...
    """
    Uniform scaling around the centroid.
    """
...
# ============================================================
# NUMERICAL INVARIANCE ERRORS
# ============================================================

translation_error = np.abs(
    original_hu -
    translated_hu
)
...
with open(
    ...
    f.write(
        "Hu Moment | Translation Error | "
        "Rotation Error | Scaling Error\n"
    )
    ...
    for i in range(7):
        ...
        f.write(
            f"Hu{i+1} | "
            f"{translation_error[i]:.12e} | "
            f"{rotation_error[i]:.12e} | "
            f"{scaling_error[i]:.12e}\n"
        )
...
for s in sorted(
    ...
    area = d["area"]
    hull_area = d["convex_hull_area"]
    solidity = d["solidity"]

    if hull_area + 1e-9 < area:

        invalid_solidity.append(
            s
        )
...
if len(invalid_solidity) == 0:

    print(
        "All Solidity values are valid."
    )

    print(
        "Condition: 0 <= Solidity <= 1"
    )

else:

    print(
        "WARNING: Invalid Solidity found."
    )
